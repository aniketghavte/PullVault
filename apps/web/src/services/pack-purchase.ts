...
export async function purchasePack(
  ...
  return db.transaction(async (tx) => {
    ...
    if (inserted) {
      ...
      // Already purchased — return early (the DB changes above are harmless
      // because the transaction will still commit, but the inventory was
      // already decremented for this purchase previously. For a truly clean
      // idempotent retry we'd need a different flow. Since idempotency_key
      // collisions mean the *same* user retried the *same* request, the
      // inventory decrement is valid.)
      //
      // NOTE: In practice, the unique constraint would have caught this on
      // the first attempt. This path handles the rare case of a retry
      // reaching step 4 again.
